...
export default function UserRatingStats({ stats }) {
  if (!stats || !stats.segments?.length) return null;

  const { segments, average, total } = stats;
  ...
  return (
    <div className="w-full mt-4 text-gray-300">
      <div className="flex justify-between items-center px-[2px] mb-2 pb-1">
        <span className="text-xs uppercase text-gray-400 tracking-widest">User Ratings</span>
        <span className="text-xs text-gray-400">{total.toLocaleString()} ratings</span>
      </div>
      ...
      <div className="mt-2 text-green-500 font-semibold text-right">
        Average: {average}
      </div>
    </div>
  );
